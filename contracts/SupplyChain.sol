...
contract SupplyChain {
    ...
    function requestPurchase(uint256 _productId) public payable onlyRole(Role.Manufacturer) {
    ...
    function confirmDelivery(uint256 _productId, uint256 recordedTemperature) public onlyRole(Role.Manufacturer) {
        Product storage product = products[_productId];

        require(!product.isSold, "Product already sold");
        require(recordedTemperature == product.temperature, "Temperature mismatch");
    ...
    function getInventoryDetails(uint256 _inventoryId) public view returns (Inventory memory) {
