...
import { fetchSalesListForFarmer } from "@/utils/firebaseHelpers";
import { ethers } from "ethers";


export default ({
    getSalesListModal,
    setGetSalesListModal,
    walletAddress,
}) => {
    ...
    const getSalesList = async () => {
        const data = await fetchSalesListForFarmer(walletAddress);
        console.log(data)
        setRequests(data);
    }

    const removeData = async () => {
        setGetSalesListModal(false)
        setRequests("")
    }

    return getSalesListModal ? (
        <div className="fixed inset-0 z-10 overflow-y-auto">
            ...
            <div className="flex items-center min-h-screen px-4 py-8">
                <div className="relative w-full max-w-3xl p-6 mx-auto bg-white rounded-md shadow-lg">
                    ...
                    <div>
                        <h2 className="text-xl font-bold mb-4">My Sales List</h2>
                        {requests.length === 0 ? (
