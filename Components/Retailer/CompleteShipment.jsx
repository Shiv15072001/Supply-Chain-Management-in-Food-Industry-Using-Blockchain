...
export default ({ completeShipmentModal,setCompleteShipmentModal, completeShipment }) => {
    const [index, setIndex] = useState(0);
    const [addedSuccessfully, setAddedSuccessfully] = useState(false);
    const [info,setInfo] =  useState("")

    const getCompleteShipment = async () => {
        if (index === 0) {
            return alert("Please fill in all the details!");
        }
        try {
            const getData = await completeShipment(index); // Set array
            console.log(getData);
            setInfo(getData);
    ...
    const removeData = () => {
        setCompleteShipmentModal(false);
            ...
            <div className='flex items-center min-h-screen px-4 py-8'>
                <div className='relative w-full max-w-3xl p-6 mx-auto bg-white rounded-md shadow-lg'>
                    ...
                    <form onSubmit={(e) => e.preventDefault()}>
                        ...
                        <button onClick={() => getCompleteShipment()} className='block w-full mt-3 py-3 px-4 font-medium text-sm text-center text-white bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 rounded-lg ring-offset-2 ring-indigo-600 focus:ring-2'>
                            Complete Shipmant
                        </button>
                    </form>
